...
class JobsController extends Controller
{
    ...
    public function addNewJob(Request $request){
                    ...
                    if ($request->input('endDate')) {
                        ...
                        if ($frequence == '0') {
                            ...
                            while (strtotime($visit_date) <= strtotime($date_ended)) {
                                ...
                                $visit->job_id = $job_id;
                                // $visit->job_type = $type;
                                $visit->member_id = $request->input('team_ids');
        ...
        if ($input['team_ids']) {
            DB::table('jobs_team')->insert(['job_id' => $job_id, 'member_id' => $input['team_ids']]);
            
            if ($request->has('notify')) {
                if ($request->input('notify') == 1 && $type == 1) {
                    $this->SendMail($user_id, $job_id, $input['team_ids']);
        ...
        foreach (explode(',', $input['team_ids']) as $team_id) {
    ...
    public function view(Request $request, $id){
    	...
    	$data['job'] = DB::table('jobs')
    		...
    		->select('jobs.*', 'jobs.job_id as job__id', 'jobs.type as job_type', 'clients.*', 'clients_properties.*', 'jobs_services.*', 'jobs_team.member_id')
    		->get();
    ...
    public function update(Request $request){
        ...
        if ($input['team_ids']) {
            if ($request->input('job_team_id')) {
                $team_ids = explode(',', $input['team_ids']);
                ...
                ->update(['member_id' => $input['team_ids']]);
            }else{
                $this->SendMail(Auth::user()->id, $request->input('job_Id'), $input['team_ids']);
                DB::table('jobs_team')->insert(['job_id' => $request->input('job_Id'), 'member_id' => $input['team_ids']]);
            }
    ...
    public function attache(Request $request){
        ...
        foreach ($request->photos as $photo) {
            ...
            $photo_object->fileID = $attachment_ID;
            $photos[] = $photo_object;
        }
