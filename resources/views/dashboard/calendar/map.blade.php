        ...
        marker.addListener('click', function () {
            ...
            google.maps.event.addListener(infoWindow, 'domready', function() {
                ...
                */
                ...
                // iwOuter.parent().css('width','275px');
                // Removes background shadow DIV
                iwBackground.children(':nth-child(2)').css({'display' : 'none'});
                iwBackground.children(':nth-child(4)').css({'display' : 'none'});
                $('.event-popover-content').css('background-color','white');
                $('.event-popover-content').find('li').eq(1).css('display','block');
                $('.event-popover-content').find('li').eq(2).css('display','flex');
                iwOuter.css('top','15px');
                iwOuter.css('background-color','white');
                iwOuter.css('border-radius','4px');

                var iwCloseBtn = iwOuter.next();
                // Apply the desired effect to the close button
                iwCloseBtn.css({opacity: '1', right: '20px', top: '15px', 'border-radius': '13px' });
